...
export default function ArticleForm({
    ...
    const submitAction = async (values) => {
        ...
        try {
            switch (mode) {
                case 'edit':
                    await request(`/articles/${articleId}`, 'PUT', articleData);
                    break;
                case 'create':
                    await request('/articles', 'POST', articleData);
                    break;
            }
    ...
    const { register, formAction, imagePreview, setValues, imageUploadRegister } = useForm(submitAction, {
    ...
    useEffect(() => {
        if (mode === 'edit') {
            request(`/articles/${articleId}`, null, null)
                .then(result => {
                                        ...
                                        {inputMethod === 'url' ? (
                                            ...
                                            <>
                                                ...
                                                <button
                                                    ...
                                                    className="w-full py-2 bg-gray-50 border-2 border-dashed border-gray-200 hover:border-[#e10600] text-gray-400 hover:text-[#e10600] text-xs font-bold uppercase tracking-wider transition-all flex items-center justify-center gap-2 rounded-sm"
                                                >
